...
- **Session chips on Pipeline cards:** every coding card shows its attached sessions (explicit `linked_sessions` ∪ fleet sessions whose `planning_refs` point back) as a `▸` chip — green pulsing **live**, **remote**/**open**, or "2h ago" for ended ones (`×N` when several). Click opens the latest session's conversation. Implementation/done cards with **no** session show a dashed `▸ no session` chip — click opens a **Code Build session seeded with the item's context** (same as the drawer's Open in Code Build). Fleet metadata refreshes every 8 s while the Pipeline is visible.
- **Drawer: Session row** — latest session (title + state) opens in one click, plus **＋ New with context** (Code Build) and **🔗 Link** (search + attach). The *Linked sessions* list now shows title — host · agent · state instead of bare UUIDs.
- **Done needs evidence:** the snapshot marks tasks/ideas with `has_screenshot` (from `kp attach`'s `<store>/media/<id>/`). Done-lane cards without one get a red **📷 missing** badge — click to attach. The move-to-done modal warns inline with a **📷 Attach now** button, and the host nudges once more ("Attach screenshot…") after a screenshot-less done move. The drawer's Screenshots section calls out done items with none.

- **AGENTS.md:** webview UI changes must be validated in a real browser (Playwright / chrome-devtools) against `test/webview/planning-harness.html` — vitest mini-DOM is not that pass. Planning styles live in `media/planning-dashboard.css` (panel + harness).
- Pipeline default route: unit + harness coverage for **Claude** (fable · medium) and **Grok** (grok-4.6 · high). Dropping onto implementation with Grok selected now launches grok (`night-orchestrator.py --implement-now`), not Claude.

## 1.59.0 — 2026-09-02

### Planning Pipeline: default route + multi-select lane moves

- **Default provider · model · effort** on the Pipeline filter bar. The choice is remembered (`kp.implRoute.last`) and applied when an unrouted card is dropped onto **implementation** (or approved). Per-item drawer routes still win.
- **Multi-select:** Ctrl/Cmd-click toggles cards; Shift-click ranges within a lane. **☑** on a lane header selects every *visible* card in that lane (search/filter respected). Drag the selection onto another lane to move them all. Esc clears. Dropping several cards onto implementation starts one auto-implement session and queues the rest.

## 1.58.0 — 2026-09-02

### Planning: coding Pipeline view (inbox → approved → implementation → done)

- New **🚀 Pipeline** view tab, separate from the Board (which keeps per-type task/idea/plan/thought lanes): **coding items only** — bugs/features (`issue_kind`), items with a `target_repo` or an implement route, and 🤖 autogenerated ideas — merged into one lifecycle: **inbox → approved → implementation → done**. Drag cards across lanes; the host maps the move per type (approve = `set-auto-implement ready` + status, and asks for a `target_repo` when missing — that's what makes it coding work; done prompts for a resolution note). Tasks/ideas only; personal items never enter the pipeline.
- **Running visibility**: the item the night implementer is building (from `planning/autonomous/plan.json` `current_window.implement`) is pulled into the implementation lane with a pulsing **▶ implementing** chip — click it to open the implementing session. Stale running markers (window long over) are ignored.
- Cards show 🐛/✨ kind, 🤖 autogenerated, ⚙ provider·model route, and underlying-status chips.
- Pipeline quick filters: kind segment (All / 🐛 Bugs / ✨ Features / 🤖 Auto), project dropdown + **⫶ by project** in-lane grouping, worked-on window (All / Today / Yesterday / Week / Month), and the lane sort select (priority / due / updated / title / project / …). All persisted in webview state.
- **Dropping a card on the implementation lane starts an auto-implement session**: the host ensures a `target_repo` + route, marks the item `in_progress`, and kicks `night-orchestrator.py --implement-now <id>` detached — the same machinery as the night flow (running state in plan.json → ▶ chip, session link, screenshots, `## Auto-implement log`, verify-gated merge). Skipped with a warning when the builder is paused (STOP) or a session is already running.
- Item drawer: **Auto-implement** section — Approve / Block / Clear plus a provider (Claude/Grok) · model · effort route stored as `implement_backend/model/effort` frontmatter. The last explicit route is remembered (globalState) and auto-applied when approving an unrouted item or starting an implementation.
- Night orchestrator: **only targets coding items** (queue head = first `kp implementable` row with `target_repo`; life/personal items flagged ready are skipped), pins that head, publishes `implement.status=running` + item + session to plan.json before the agent starts, honors per-item model/effort routes, and links the implementing session to the item. The implement prompt now restricts selection to `target_repo` items, marks them `in_progress` on pickup, captures screenshots (`kp attach`), and appends an `## Auto-implement log` entry with implementation details.
