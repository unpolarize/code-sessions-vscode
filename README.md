...
- **Auto-memory worktree silos** — Claude Code stores auto-memory at `~/.claude/projects/<dash-encoded-cwd>/memory/MEMORY.md` (every non-alphanumeric character in the cwd becomes `-`). Each git worktree, including `.claude/worktrees/<name>`, is therefore a **separate silo** while you probably wanted repo-scoped memory. The card appears when ≥2 silos share a git common-dir or a `--claude-worktrees-` / `--git-worktrees-` encoded stem; it lists diverging cwds with size / ~token estimates and offers **Copy merge candidates** + **Open silo paths**. Read-only — never deletes memory. Dash-decode is lossy when the real path contains hyphens. Codex `~/.codex/memories/` is global (not per-cwd) so it does not fragment; OpenCode analog unknown in v1.
- **Compaction fidelity leaderboard** — ranks backends by how many KP **Acceptance** bullets are still recoverable after compact events (substring heuristic; unlinked sessions excluded). Minimum sample filter so empty corpora cannot fake ranks; 1–3 example sessions per backend; **Export JSON** for night reports. Hidden when the window has no KP-linked compact events. Read-only — never triggers a compact.
...
- `~/.claude/projects/<dash-encoded-cwd>/memory/` — Claude auto-memory silos (Insights doctor; cwd encoding replaces non-alphanumeric chars with `-`, so worktrees fragment)
