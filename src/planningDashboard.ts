...
function renderInbox(){
  ...
  rows.forEach(o=>{
    ...
    c.innerHTML='<div class="sh"><span class="ct">'+esc(o.title||o.id)+'</span><span class="cm"><span class="badge">'+o.type+'</span>'+auto+(o.domain?'<span>'+esc(o.domain)+'</span>':'')+prov+'</span></div>';
...
function renderAutonomous(){
  ...
  el2.appendChild(el('div',null,'<div style="font-size:14px;font-weight:700">🤖 Autonomous builder</div><div style="opacity:.65;font-size:12px;margin:2px 0 14px">Overnight ideate→spec→implement, aligned to 5-hour Claude windows. Ideas land in the Inbox tagged <b>autogenerated</b>.</div>'));
  if(!a){ el2.appendChild(el('div',null,'<div style="opacity:.6;padding:14px 2px">No autonomous run yet. The night orchestrator writes its plan here after its first window (planning/autonomous/plan.json).</div>')); }
    ...
    // 2) projected usage + config
    const cw=a.current_window;
    const usage=el('div','autorow');
    usage.innerHTML='<div class="autostat"><span class="l">Projected usage / window</span><span class="v">'+esc(String(a.projected_usage_pct||0))+'%</span></div>'+
      '<div class="autostat"><span class="l">Window</span><span class="v">'+esc(String(a.window_hours||5))+'h · implement '+esc(String(a.implement_lead_hours||1))+'h before close</span></div>'+
      '<div class="autostat"><span class="l">Active band</span><span class="v">'+esc(a.active_band||'—')+'</span></div>'+
    ...
    const meter=el('div','usagebar'); const fill=el('div','usagefill'); fill.style.width=Math.min(100,(a.projected_usage_pct||0))+'%'; meter.appendChild(fill); sched.appendChild(meter);
    if(cw){
      const id=cw.ideate||{}, im=cw.implement||{};
      sched.appendChild(el('div','autoline','<span class="ph ide">ideate</span> window '+fmtDT(cw.start)+' → '+fmtDT(cw.end)+' · <b>'+esc(id.status||'')+'</b>'+(id.created_count?' · '+id.created_count+' ideas':'')+(id.spec?' · '+esc(id.spec):'')));
    ...
    reps.forEach(r=>{ const row=el('div','autoline'); row.innerHTML='<span class="ph rep">report</span> '+fmtDT(r.when)+' · <span class="lnk">'+esc(r.report)+'</span>'+(r.status?' · '+esc(r.status):''); row.querySelector('.lnk').addEventListener('click',()=>vscode.postMessage({type:'open',kbPath:r.report})); rs.appendChild(row); });
    el2.appendChild(rs);
  }
  // 4) ideas/tasks created during ideation (autogenerated) — always show what exists
  const ideas=autoIdeas();
  ...
  const list=el('div','sociallist');
  ideas.forEach(o=>{ const c=el('div','socialcard'); c.innerHTML='<div class="sh"><span class="ct">'+esc(o.title||o.id)+'</span><span class="cm"><span class="badge">'+o.type+'</span>'+(o.status?'<span>'+esc(o.status)+'</span>':'')+(o.surfaced_on||o.created?'<span>'+esc(o.surfaced_on||o.created)+'</span>':'')+'</span></div>'; c.addEventListener('click',()=>openDetail(o.id)); list.appendChild(c); });
